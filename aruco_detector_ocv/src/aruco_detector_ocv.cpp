...
if(ids.size()>0)
{
	...
	if (result_img_pub_.getNumSubscribers() > 0)
	{
                cv::putText(display_image, ""+SSTR(image_width)+"x"+SSTR(image_height)+"@"+SSTR(image_fps)+"FPS m. size: "+SSTR(marker_size)+" m", cv::Point(10, 30), cv::FONT_HERSHEY_SIMPLEX, 0.8, CV_RGB(255, 255, 0), 2);
...
int main(int argc, char **argv) {
    ...
    nh.param("camera", rgb_topic, string("/rgb/image_raw"));
    nh.param("camera_info", rgb_info_topic, string("/rgb/camera_info"));
    nh.param("show_detections", show_detections, true);
    nh.param("tf_prefix", marker_tf_prefix, string("marker"));
    nh.param("marker_size", marker_size, 0.05f);
    nh.param("image_fps", image_fps, 10);
    nh.param("image_width", image_width, 1280);
    nh.param("image_height", image_height, 720);
    nh.param("num_detected", num_detected, 5);
    nh.param("min_prec_value", min_prec_value, 80);

    detector_params = aruco::DetectorParameters::create();
    detector_params->cornerRefinementMethod = aruco::CORNER_REFINE_SUBPIX;
    nh.param("dictionary_name", dictionary_name, string("DICT_6X6_100"));
